...
<div class="divEnroll">
   ...
   <form id="updateForm" action="${ path }/merchant/hobbyUpdate" method="POST"
      enctype="multipart/form-data">

      <input type="hidden" name="merNo" value="${ loginMerchant.merNo }" /> 
      <input type="hidden" name="hbNo" value="${ hobby.hbNo }" id="hbNo" /> 

      <div class="enroll enroll-1 active">
         <section>
            <div class="title">
               <span>타이틀</span> 
               <input type="text" id="hbTitle" name="hbTitle" class="req" value="${ hobby.hbTitle }"> 
               
               <span>간략소개</span>
               <input type="text" id="hbSummary" name="hbSummary" class="req" value="${ hobby.hbSummary }">
               <span>카테고리</span>
               <select id="cateNo" name="cateNo" class="req" >      
                          <c:forEach var="category" items="${ list }">
                          <c:if test="${hobby.cateNo == category.cateNo }">
                         <option value="${ category.cateNo }" selected="selected">
            ...
            <div class="title">
               ...
               <span>취미 URL</span> <input type="text" id="hbUrl" name="hbUrl"
                  ...
                  <div class="file-upload preview-image">
                        ...
                        <img src="${path }/resources/upload/hobby/${hobby.hbThumRename}" class="upload-thumb">
                        </div></div>
                        <input type="text" class="upload-name" value="${ hobby.hbThumOri }" disabled="disabled"> 
               ...
               <span>추가 이미지</span>
               <c:if test="${ hobby.hbImgsOri != null }">
               <input type="hidden" value="${hobby.hbImgsRename}" id="hbImgs">
              <button type="button" id="imgsDelete" style=" margin: 10px 10px 10px 0px; cursor: pointer; height: 24px; width: 90px; border: 1px solid #333; text-align: center; font-weight: bold; font-size: 10px;">
              추가 이미지 삭제</button>
               <c:out value="(추가이미지 기존파일 제거를 원할 시 버튼을 눌러주세요.)"></c:out>
               </c:if>
      ...
      <div class="enroll enroll-3">
         <section>
            <div class="title">
               <span>취미 시작일</span> 
               <input type="date" id="hbStartDate" name="hbStartDate" min="${today}" class="req" value="${ hobby.hbStartDate }"> 
               <span>취미 종료일</span> 
               <input type="date" id="hbEndDate" name="hbEndDate" min="${today}" class="req" value="${ hobby.hbEndDate }">
      ...
      <div class="enroll enroll-4">
         <section>
            <div class="title">
               ...
               <input type="text" id="hbFee" name="hbFee" value="${hobby.hbFee }" class="req"> 
               <span>취미 할인여부</span>
               <div id="disStat">
                  <label>할인 적용</label> 
                  <c:if test="${ hobby.hbDiscountStatus eq 'Y' }">
                  <input type="checkbox" id="hbDiscountStatus"
                     name="hbDiscountStatus" value="Y" checked="checked"/>
                  <c:out value="(추후에 변경이 가능합니다.)"></c:out>
                  <div id="disRate">
                     <span>취미 할인률</span> 
                     <input type="text" id="hbDiscountRate" name="hbDiscountRate" class="reqDisRate" value="${ hobby.hbDiscountRate }">
                  </div>
                  </c:if>
      ...
      <div class="enroll enroll-5">
         <section>
            <div class="title">
               ...
               <div class="upload-display-detail">
               <div class="upload-thumb-wrap-detail">
               <img src="${path }/resources/upload/hobby/${hobby.hbDetailRename}" class="upload-thumb-detail"></div></div>
                   <input type="text" class="upload-name-detail" value="${ hobby.hbDetailOri }" disabled="disabled"> 
               ...
               <span>설명</span> 
               <textarea id="detail" name="hbDetailInfo" class="req" cols="67" rows="15" style="resize: none;">
               	<c:out value="${ hobby.hbDetailInfo }"></c:out>
               </textarea>
      ...
      <div class="enroll enroll-6">
         <section>
            <div class="title">
               ...
               <span>설명</span> 
               <input type="text" id="hbThumUrl" name="hbInfo" class="req" value="${ hobby.hbInfo }"> 
               <span>주소 입력하기</span> 
               <input type="hidden" value="${ hobby.hbLocation }" id="address">
               <input type="text" class="req" name="postcode" id="postcode" /> 
               <input type="button" value="주소 찾기" id="hbLocationButton" onclick="sample4_execDaumPostcode();"/> 
               <input type="text" class="req" name="hbLocation" id="roadAddress"  />
               <input type="text" class="req" name="exactAddress" id="exactAddress" />

               <div class="btn">
                  <input type="button" class="before" value="이전으로" /> 
                  <input type="button" class="register" value="수정하기" />
               </div>
            </div>
            <div class="warn">
               <p>
				...
				for(var i in addrSplit){
					$('#postcode').val(addrSplit[0]);
					$('#roadAddress').val(addrSplit[1]);
					$('#exactAddress').val(addrSplit[2]);
				}
				

					
				 var imgsRename = $('#hbImgs').val() ;
				 var imgSplit = imgsRename.split(',');
				 for(var i in imgSplit){
					
					 
					 var imgs = '<img src="${ path }/resources/upload/hobby/'+ imgSplit[i] +'">';
					 console.log(imgs);
					 $('#att_zone').append(imgs);
