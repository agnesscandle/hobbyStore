...
<span class="mainLabel"><h2>좋아요 취미</h2></span>
<div class="divList_h">
...
<div id="pageBar">
   <!-- 맨 처음으로 -->
   <button onclick="location.href='${path}/member/likedHobby?page=1'">&lt;&lt;</button>

   <!-- 이전 페이지로 -->
   <button
      onclick="location.href='${path}/member/likedHobby?page=${ pageInfo.prvePage }'">&lt;</button>
   ...
   <c:forEach begin="${pageInfo.startPage}" end="${pageInfo.endPage}"
      ...
      <c:if test="${pageInfo.currentPage != status.current }">
         <button
            onclick="location.href='${path}/member/likedHobby?page=${status.current}'">
            <c:out value="${status.current}" />
         </button>
      </c:if>
   </c:forEach>
   <!-- 다음 페이지로 -->
   <button
      onclick="location.href='${path}/member/likedHobby?page=${pageInfo.nextPage}'">&gt;</button>

   <!-- 맨 끝으로 -->
   <button
      onclick="location.href='${path}/member/likedHobby?page=${pageInfo.maxPage}'">&gt;&gt;</button>
</div>
